...
export default function CreateFunnel({ editor }: PageProps) {
	return (
		<>
			<Head title={`New Funnel · ${editor.website.name}`} />
			<main className="mx-auto w-full max-w-7xl flex-1 px-4 py-8 sm:px-6 lg:px-8">
				...
				<Form route="funnels.store" routeParams={{ id: editor.website.id }}>
					{({ errors, processing }) => (
						<FunnelForm editor={editor} errors={errors} processing={processing} submitLabel="Create Funnel" />
					)}
				</Form>
			</main>
		</>
